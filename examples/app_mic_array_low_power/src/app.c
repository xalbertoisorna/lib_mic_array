...
{    
    energy_ctx_t ctx = {0};
    float den0 = (float)(PLL0_OD + 1);
    float den1 = (float)(XCORE1_DIV + 1);
    float den2 =  (float)(REF_DIV + 1);

    float in_freq = 600.0f;
    float tree_freq = in_freq / den0;
    float tile_freq = tree_freq / den1;
    float ref_freq = tree_freq / den2;
    
    printf("----------------------------\n");
    printf("PLL0_OD: %d\n", PLL0_OD);
    printf("XCORE1_DIV: %d\n", XCORE1_DIV);
    printf("TREE FREQ: %f MHz\n", tree_freq);
    printf("TILE FREQ: %f MHz\n", tile_freq);
    printf("REF FREQ: %f MHz\n", ref_freq);
    printf("----------------------------\n");

    if(ref_freq < tile_freq)
    {
        printf("Warning: Reference clock frequency is lower than tile frequency!\n");
    }
    
    if(ref_freq != 100.0f)
    {
        printf("Warning: Reference clock is not 100MHz!\n");
    }

    ctx.freq_mhz = ref_freq;
    //energy_measure_init();
    power_mgmt_setup();
    //energy_measure_begin(&ctx);
    ...
    while (frame_counter--)
    {
        ...
        ma_frame_rx(
            buff_ptr, c_mic_audio, MIC_ARRAY_CONFIG_MIC_COUNT, APP_N_SAMPLES
        );
        t1 = hwtimer_get_time(tmr);
        buff_ptr += APP_N_SAMPLES;
    }
    //energy_measure_end(&ctx);
    printf("cycles ma %d\n", (t1 - t0));

    // write samples to a binary file
    power_clock_pll0_set_OD(GEN_DIV_0);
    power_tile_set_divider(tile1, GEN_DIV_0);
